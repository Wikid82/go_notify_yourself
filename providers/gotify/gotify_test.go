...
const validURL = "https://gotify.example.com/message"
...
func TestClientSendSendsGotifyKeyHeader(t *testing.T) {
	rt := &capturingRoundTripper{}
	client := New(Config{URL: validURL, Token: "secret-token"}, newTestWrapper(rt))

	if err := client.Send(context.Background(), notify.Message{Title: "t", Body: "hello"}); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if got := rt.lastRequest.Header.Get("X-Gotify-Key"); got != "secret-token" {
		t.Fatalf("expected X-Gotify-Key header, got %q", got)
	}
}

func TestClientSendOmitsGotifyKeyHeaderWhenTokenEmpty(t *testing.T) {
	rt := &capturingRoundTripper{}
	...
	if got := rt.lastRequest.Header.Get("X-Gotify-Key"); got != "" {
		t.Fatalf("expected no X-Gotify-Key header, got %q", got)
	}
}
