...
// init registers this package's Factory under the name "gotify" with the
// notify package's registry.
//
// Expected config keys:
//   - "transport" (required): *transport.Wrapper — the shared dispatch
//     primitive; see transport.NewWrapper.
//   - "url" (string): the Gotify server's message push endpoint.
//   - "token" (string, optional): the Gotify application token.
//   - "template" (string, optional): "minimal" (default), "detailed", or
//     "custom".
//   - "custom_template" (string, optional): used when template is "custom".
func init() {
	notify.Register("gotify", func(config map[string]any) (notify.Sender, error) {
		w, ok := config["transport"].(*transport.Wrapper)
		if !ok || w == nil {
			return nil, fmt.Errorf(`gotify: config["transport"] must be a non-nil *transport.Wrapper`)
		}
		cfg := Config{
